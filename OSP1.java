                ...
                {                                //Returns false if no 0, true if there is a 0                      
                    if (disk.isEmpty() == false) {
                        if (disk.size() == 300) {
                            ...
                            do 
							{
                                StringBuffer appendLoadTime = new StringBuffer(disk.element()); //Prepares to append load time to job if it is chosen to be loaded into ready queue
                                String tempItem = disk.element();
                                if (sched.aquireMemoryCheck(tempItem)) 
								{
                                    appendLoadTime.append("	" + cpuTime); //Appends load time onto job line, adds it to ready queue to await processing.
